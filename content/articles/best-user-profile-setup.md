...
When creating a new user profile you go through the setup wizard similar to when first setting up the phone. The new user profile has the same default apps as the owner profile and no user data.
...
One advantage to this setup is that if a personal profile needs to be deleted for any reason, it's easy to do so, either from within the specific profile itself, or from the owner profile. There's no need to do a factory reset, and any other profiles that might be on the phone won't be affected.
